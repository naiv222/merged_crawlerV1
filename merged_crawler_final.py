...
def extract_reviews_from_detail(page: Page) -> List[Dict[str, str]]:
    """리뷰 영역에서 별점/내용/작성일 추출"""
    reviews: List[Dict[str, str]] = []
    items = page.locator("li.danawa-prodBlog-companyReview-clazz-more")
    count = items.count()
    ...
    for i in range(count):
        ...
        # 점수 (예: '100점')
        try:
            score_text = item.locator(".point_type_s .star_mask").inner_text().strip()
        except Exception:
            score_text = ""
        score = 0
        ...
        # 날짜
        try:
            date = item.locator(".top_info .date").inner_text().strip()
        except Exception:
            date = ""

        # 내용 (요청: atc)
        try:
            content = item.locator(".atc_cont .atc").inner_text().strip()
        except Exception:
            content = ""
...
def collect_option_links_from_detail(page: Page) -> List[Dict[str, str]]:
    """
    상세 페이지 내 '다른 구성' 영역의 옵션 리스트(li.list-item)에서
    옵션 링크와 스펙 텍스트(예: '10개', '20개')를 수집.
    """
    options: List[Dict[str, str]] = []
    option_items = page.locator("div.bundleProductListArea li.list-item")
    count = option_items.count()

    for i in range(count):
        li = option_items.nth(i)
        try:
            href = li.locator("a.link__full").get_attribute("href")
        except Exception:
            href = None
        if not href:
            continue
        ...
        spec_text = ""
        try:
            spec_text = li.locator(".text__spec").inner_text().strip()
        except Exception:
            pass
...
def transform_specs_to_detail_info(specs: Dict[str, str], learned_mapping: Dict[str, str]) -> str:
    """
    기존 PASS2에서 사용하던 스펙 플래턴 로직을 함수로 분리.
    specs + learned_mapping을 받아 detail_info 문자열을 생성.
    """
    spec_parts: List[str] = []
    certification_items: List[str] = []       # 인증 타입 (예: HACCP인증, 적합성평가인증)
    certification_info_items: List[str] = []  # 인증 세부정보 (예: 인증번호)
    registration_date: str = ""              # 등록년월일
    ...
    for key, value in specs.items():
        ...
        is_meaningless = clean_value in meaningless_values or any(
            mv in clean_value for mv in ["상세설명 / 판매 사이트 문의"]
        )
...
def crawl_option_detail_and_reviews(
    context: BrowserContext,
    option_url: str,
    option_spec: str,
    base_url: str,
    learned_mapping: Dict[str, str],
    base_delay_ms: int,
    all_rows: List[Dict[str, str]],
) -> None:
    """
    옵션 상세 페이지에 접속해서:
    1) #bookmark_product_information 에서 상세 정보 수집
    2) #bookmark_cm_opinion 에서 리뷰 수집
    리뷰가 여러 개면 리뷰 1개당 CSV 1행.
    """
    detail_page = context.new_page()
    detail_page.set_default_timeout(15000)
    ...
    try:
        # 1) 상세정보 탭으로 이동
        detail_url = option_url + "#bookmark_product_information"
        print(f"      [옵션] 상세정보 크롤링: {detail_url}")
        ...
        # 옵션 페이지의 상품명(title) = 옵션명
        try:
            title = detail_page.title() or ""
        except Exception as e:
            print(f"        경고: 옵션 제목 추출 실패 - {e}")
            title = ""

        specs = extract_specs_from_detail(detail_page)
        detail_info = transform_specs_to_detail_info(specs, learned_mapping)

        # 2) 리뷰 탭으로 이동
        review_url = option_url + "#bookmark_cm_opinion"
        print(f"      [옵션] 리뷰 크롤링: {review_url}")
        try:
            detail_page.goto(review_url, wait_until="domcontentloaded", timeout=15000)
            wait_for_network_idle(detail_page)
            slow_scroll(detail_page, steps=2, step_px=900, base_delay_ms=base_delay_ms)
        except Exception as e:
            print(f"        경고: 리뷰 탭 이동 실패 - {e}")

        reviews = extract_reviews_from_detail(detail_page)
        ...
        print(f"      [옵션] 완료 - 리뷰 {len(reviews)}개 수집")

    except Exception as e:
        print(f"      오류: 옵션({option_url}) 크롤링 실패 - {e}")
    finally:
        try:
            detail_page.close()
        except Exception:
                ...
                for idx, link in enumerate(product_links, 1):
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 기본 상품 기준 상세정보도 한 번 수집 (원하면 필터링 가능)
                            specs = extract_specs_from_detail(detail_page)
                            title = ""
                            try:
                                title = detail_page.title() or ""
                            ...
                            # 기본 상품에 대한 detail_info
                            detail_info = transform_specs_to_detail_info(specs, learned_mapping)

                            # 기본 상품 행도 추가(리뷰 없는 루트 상품)
                            base_row = {
                                "BASE_URL": link,
                                "현재URL": link,
                                "상품명": title,
                            ...
                            print(f"    기본 상품 정보 수집 완료 (총 {len(all_rows)}개 행)")

                            # === 옵션 및 리뷰 크롤링 추가 부분 ===
                            try:
                                options = collect_option_links_from_detail(detail_page)

                                if options:
                                    print(f"    옵션 {len(options)}개 발견 - 옵션/리뷰 크롤링 시작...")
                                    for opt_idx, opt in enumerate(options, 1):
                                        print(f"      옵션 {opt_idx}/{len(options)}: {opt['url']}")
                                        crawl_option_detail_and_reviews(
                                            context=context,
                                            option_url=opt["url"],
                                            option_spec=opt["spec"],
                                            base_url=link,
                                    ...
                                    print("    옵션 없음 → 기본 상품에서 리뷰 크롤링 실행")

                                    # 기본 상품 리뷰 탭 이동
                                    review_url = link + "#bookmark_cm_opinion"
                                    try:
                                        detail_page.goto(review_url, wait_until="domcontentloaded", timeout=15000)
                                        wait_for_network_idle(detail_page)
                                        slow_scroll(detail_page, steps=2, step_px=900, base_delay_ms=base_delay_ms)
                                    except Exception as e:
                                        print(f"    경고: 기본 상품 리뷰 탭 이동 실패 - {e}")
